...
module.exports = {
 ...
 role: 0,
 description: {
 en: "Give someone a tuntun effect",
 vi: "Tạo hiệu ứng tuntun cho ai đó"
 },
 category: "fun",
 guide: {
 en: ' {pn} <@tag>: Give tuntun effect by tagging'
 + '\n {pn} <uid>: Create effect using UID'
 + '\n (Or use by replying to a message)',
 vi: ' {pn} <@tag>: Tạo hiệu ứng tuntun bằng cách gắn thẻ'
 ...
 const filePath = path.join(cacheDir, `tuntun_${id2}.png`);

 try {
 if (!fs.existsSync(cacheDir)) fs.mkdirSync(cacheDir);

 const response = await axios.get(`${await mahmud()}/api/fun?type=tuntun&user=${id2}`, { responseType: "arraybuffer" });
